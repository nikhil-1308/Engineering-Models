...
controls = [v;omega]; n_controls = length(controls);
rhs = [v*cos(theta);v*sin(theta);omega]; % system r.h.s
f = Function('f',{states,controls},{rhs}); % MOTION MODEL & nonlinear mapping function f(x,u)

r = SX.sym('r'); alpha = SX.sym('alpha'); % range and bearing
measurement_rhs = [sqrt(x^2+y^2); atan(y/x)];

% rhs of mesurement model & can use it directly as it is a algibric equation
...
% no of rwos^2, N_MHE+1(columns)=measurement output, N_MHE(columns)=measuremnt input of control action
% we have 2 outputs and 2 inputs that is why 2 rows
...
% %           measurement alpha,           nominal control action applied
% % initial value of the optimization variables
...
%% MHE IN RECIDING HORIZON MANNER
