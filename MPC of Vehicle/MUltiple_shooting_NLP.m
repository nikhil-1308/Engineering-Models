...
args.lbg(1:3*(N+1)) = 0;  % ever shooting step has three states [a;b;c] % -1e-20  % Equality constraints
...
while(norm((x0-xs),2) > 1e-2 && mpciter < sim_tim / T)
    ...
    xx1(:,1:3,mpciter+1)= reshape(full(sol.x(1:3*(N+1)))',3,N+1)'; % minimized cost funtion output x(k+1)=f(xu(k),u(k))
...
% figure(1)
% subplot(311)
% plot(x0(:,1),'b','linewidth',1.5); hold on
% grid on
% legend('Ground Truth','Measurement')
% subplot(312)
% plot(u(:,2),'b','linewidth',1.5); hold on
% grid on
